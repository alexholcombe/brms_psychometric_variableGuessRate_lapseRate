...
# Specify distribution names and parameters separately
lapseRate_prior_dist_name <- "beta"
lapse_param1 <- 2
lapse_param2 <- 33.33

logsigma_prior_dist_name <- "uniform"
logsigma_param1 <- -3
logsigma_param2 <- 1.6
...
logsigma_prior_distribution <- sprintf("%s(%s, %s)", logsigma_prior_dist_name,
                                       logsigma_param1, logsigma_param2)
...
#eta_prior_distribution<-  sprintf("uniform(%s, %s)", eta_uniform_param1, eta_uniform_param2)
eta_prior_distribution<-  sprintf("normal(%s, %s)", eta_gaussian_param1, eta_gaussian_param2)

eta_differences_prior_distribution<- sprintf("normal(%s, %s)", 
                                      eta_diffs_gaussian_param1,
                                      eta_diffs_gaussian_param2)
...
my_priors <- c( lapseRate_prior, eta_prior, logSigma_prior,
                ageGroup_prior, lab_prior)
```

Visualize priors, by plotting together (with arbitrary height).
...
prior_location <- tibble(
  x = seq(-1, 10, length.out = 500),
  y = dunif(x, eta_uniform_param1, eta_uniform_param2),
  parameter = "location"
) |>
...
prior_location_diff <- tibble(
  x = seq(-4, 10, length.out = 500),
  y = dnorm(x, eta_diffs_gaussian_param1, eta_diffs_gaussian_param2),
  parameter = "location group difference"
) |>
  mutate(y = y / max(y) * 0.95) #Normalize so has peak of 0.95 (to avoid overlap)
...
print( summary(fit) )
```
## Interpreting the intercepts and coefficients estimated by brms
...
trueValsNamesInBrmParlance<-
  c("eta_labRoudaia","eta_ageGroupyounger","eta_numTargetstwo","eta_genderF", "lapseRate_Intercept","logSigma_Intercept") #,"logSigma_ageGroupyounger")
trueVal<-c( 
