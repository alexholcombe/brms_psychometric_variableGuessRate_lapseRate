...
Show stan code . But I don't know how to interrogate it to see the factors.
```{r}
#| echo: false 
...
time <- try( system.time({ 
  fit <-
    brm(
      ...
      init = 0, #starting at 0 recommended "if chains do not initialize or behave well"
      chains = 2,#3, #4 is the default to check consistent convergence
      iter = 200 #2000 is default number of iterations 
    ) 
  }), #end of system.time
...
chainchainchain |>
  ...
  labs(title = "Posterior Density for gender")
```
...
prediction_grid <- expand.grid(
  ...
  ageGroup = unique(data_simulated$ageGroup),
  gender = unique(data_simulated$gender),
  numTargets = unique(data_simulated$numTargets),
...
prediction_subset <- prediction_conditions %>%
  filter(numTargets=="two", lab == "Roudaia", gender=="F",obj_per_ring==5)
...
data_subset <- data_simulated %>%
  filter(numTargets=="two", lab == "Roudaia", gender=="F",obj_per_ring==5, subj_standardzd == 0)

# Plot
ggplot(prediction_subset, aes(x = speed, y = p_correct, color = ageGroup)) +
  geom_line() +
...
hh<- ggplot(prediction_conditions, 
       aes(x = speed, y = p_correct, color = factor(numTargets), linetype = ageGroup)) +
  geom_line() +
  geom_ribbon(aes(ymin = p_lower, ymax = p_upper, fill = factor(numTargets)), alpha = 0.4, color = NA) +
  facet_grid(lab ~ obj_per_ring) +
  labs(
    x = "Speed (revolutions per second)",
