...
one_less_target_benefit<-0.3 #0.15 #penalty for additional target
youth_benefit <- 0.2#0.4 #Old people have worse limit by this much
male_benefit <- 0.15 #0.12 #Female worse by this much
Roudaia_lab_benefit <- 0.2
...
my_priors <- c( lapseRate_prior, logSigma_prior, eta_prior, #Intercept priors
...
Next, will really call brm to fit the model! Which could take a very long time to execute.
...
cat('Time taken (min) =', round( timeToRun["elapsed"] / 60. ,1) )
...
trueVal<-c( 
     Roudaia_lab_benefit,youth_benefit,#target_benefit,male_benefit, 
     lapse, log(sigma))#, 0)
...
chainchainchain |>
  ...
  facet_grid(paramGroup ~ ., scales="free") +
  geom_line() +
  geom_hline(yintercept=youth_benefit, alpha=.5,linetype="dotted")+
  geom_hline(yintercept=male_benefit, alpha=.5,linetype="dotted")+
  ggtitle("")+
  theme_bw()
