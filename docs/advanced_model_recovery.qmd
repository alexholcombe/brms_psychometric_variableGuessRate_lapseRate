...
location_param_base<-1.8 #location_param_young_123targets <- c(1.7,1.0,0.8)
target_penalty<-0.15

age_penalty <- 0#0.4 #Old people have worse limit by this much
gender_penalty <- 0#0.1 #Female worse by this much
Holcombe_lab_penalty <- 0#0.2
...
location_param_calculate<- function(num_targets,age_group,gender,lab,
  ...
  base_location_param <- location_param_base#location_param_young_123targets[num_targets]
...
data_simulated<- data_simulated |> mutate( target_load = 
                                      if_else(num_targets==2,"less","more") )
...
gg<- ggplot(data_simulated, #data_one_condition, 
            aes(x=speed,y=p_correct,linetype=age_group,color=factor(target_load))) +
...
gg<-gg+stat_summary( aes(group=interaction(age_group,target_load)),
...
loc_param_each_group <- data_simulated |>
        group_by(age_group, lab, target_load) %>%
...
my_brms_formula <- brms::bf(
  correct ~ chance_rate + (1-chance_rate - lapseRate * (1-chance_rate))*Phi(-(speed-eta)/exp(logSigma)), 
  eta ~ target_load,# age_group + num_targets + lab,#  + gender
...
trueValsNamesInBrmParlance<-
  c("eta_Intercept","eta_age_groupyounger","eta_genderM", "eta_labRoudaia", "lapseRate_Intercept","logSigma_Intercept") #,"logSigma_age_groupyounger")
trueVal<-c( 
     true_eta_reference, age_penalty, gender_penalty, Holcombe_lab_penalty,
     lapse, log(sigma))#, 0)
...
rounded<- estimates %>% dplyr::mutate(across(where(is.numeric), ~ round(.x, 2)))
print(rounded)

if (all(est_no_sigma$outside_CI==0)) {
  print('All estimates (other than logSigma estimations) are within confidence interval of true value!')
} else {
