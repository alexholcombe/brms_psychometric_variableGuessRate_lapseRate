...
```{r}
#| echo: true 
numTargets<-c(2,3) #laboratories<- c("Holcombe","Roudaia")
numSubjects<- 25
trialsPerCondition<- 8
targetLoads<-c("not used","not used") #targetNumConds<- c(2,3)
...
trials <- generate_conditions(numTargets,numSubjects,trialsPerCondition,targetLoads,speeds)
...
my_brms_formula <- brms::bf(
  correct ~ chance_rate + (1-chance_rate - lapseRate * (1-chance_rate))*Phi(-(speed-eta)/exp(logSigma)), 
  eta ~ age_group + numTargets,#lab,#  + gender
