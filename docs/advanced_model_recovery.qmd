...
gender_penalty <- 0.1 #Female worse by this much
...
my_brms_formula <- brms::bf(
  correct ~ chance_rate + (1-chance_rate - lapseRate * (1-chance_rate))*Phi(-(speed-eta)/exp(logSigma)), 
  eta ~ age_group + gender + lab,# + num_targets, 
...
#So I need to calculate that reference value
eta_reference_group <- data_simulated |> filter(lab=="Holcombe",age_group=="older",
                                                gender=="F", num_targets==2)
eta_reference<- eta_reference_group |> 
                  summarise(eta_avg = mean(location_param), .groups="drop")
eta_reference<- mean(eta_reference$eta_avg)
print(eta_reference)

# ground truth overall eta
# The easiest way might be to group my data_simulated by all the factors and average those
# location_param together
eta_each_group <- data_simulated |>
              group_by(lab,age_group,gender,num_targets) |>
              summarise(eta_avg = mean(location_param), .groups="drop")
true_eta_Intercept = mean(eta_each_group$eta_avg)
...
trueVal<-c( 
     true_eta_Intercept, age_penalty, gender_penalty, Holcombe_lab_penalty,
     lapse, log(sigma))#, 0)
