...
trials <- generate_conditions(laboratories,numSubjects,trialsPerCondition,targetLoads,speeds)
...
location_param_calculate<- function(targetLoad,age_group,gender,lab,
  ...
  after_penalties <-location_param - 
                      ...
                      if_else(targetLoad=="more",1,0) * target_penalty
  return (after_penalties)
}

#Need version with fixed eta_between_subject_sd for use in mutate
location_param_calc_for_mutate<- function(targetLoad,age_group,gender,lab,
                                               subjStandardized) {
  location_param_calculate(targetLoad,age_group,gender,lab,
                           subjStandardized, eta_between_subject_sd)
}
...
data_simulated <- data_simulated %>%
  mutate(
    ...
    location_param = location_param_calc_for_mutate(targetLoad,age_group,gender,
                                                    lab,subj_standardzd),
...
gg<- ggplot(data_simulated, #data_one_condition, 
            aes(x=speed,y=p_correct,linetype=age_group,color=factor(targetLoad))) +
...
loc_param_each_group <- data_simulated |>
        group_by(age_group, lab, targetLoad) %>%
...
my_brms_formula <- brms::bf(
  correct ~ chance_rate + (1-chance_rate - lapseRate * (1-chance_rate))*Phi(-(speed-eta)/exp(logSigma)), 
  eta ~ targetLoad + age_group,# + lab,#  + gender
