...
set.seed(997) #ensures reproducibility for testing
...
data_subset$curve_type <- "generating model"
# Combine for plotting
plot_data <- bind_rows(
  prediction_subset %>% select(speed, p_correct, curve_type),
  data_subset %>% select(speed, p_correct, curve_type)
)

hh<- ggplot(plot_data, aes(x = speed, y = p_correct, color = curve_type)) +
  geom_line() + #fit
  #geom_ribbon(aes(ymin = p_lower, ymax = p_upper), fill = "blue", alpha = 0.2, color = NA) +
  geom_line(data = data_subset, #actual data generating model
            linetype="dotted") + geom_point(data=data_subset) +
  # labs(
