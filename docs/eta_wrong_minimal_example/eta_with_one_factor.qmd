...
example_condition <- data.frame(
  ...
  ageGroup = c("younger","older"), #first level
  gender = "M", #first level
  numTargets = "two", #first level
...
#| echo: false
  ...
  trials<-trials |> 
    mutate( subjStandardzd = center_on_zero_and_standardize_std(subjWithinGroup,
...
location_param_base<-1.5 #location_param_young_123targets <- c(1.7,1.0,0.8)
target_penalty<-0#0.9 #penalty for additional target
age_penalty <- 0.6#0.4 #Old people have worse limit by this much
...
eta_between_subject_sd <- .001#0.2 
...
For this minimal example, model only one factor and only use data relevant to that
...
loc_param_each_group <- data_to_model |>
...
data_to_model$logSigma<- log(sigma)
#and don't model lapse, supply it in the data
data_to_model$lapseRate <- lapse
...
# Combine all priors
priors_all <- bind_rows(prior_location) #,prior_lapse)#, prior_scale)
...
if (break_brms_by_making_numTargets_numeric) {
  ...
  #Delete old column
  data_to_model$numTargets <- NULL #delete column
  data_to_model <- data_to_model %>%
...
data_eta<- data_to_model$location_param
...
prediction_grid <- expand.grid(
  ...
  subjStandardzd = 0         # Use mean subject
)
...
#Not modeling sigma, so need to add it to prediction so prediction can be made
...
#calculate the *true* threshold percent correct level, which differs by num_objects as well as lapse
loc_param_each_group <- data_to_model |>
        group_by(ageGroup, lab, numTargets) %>%
        summarise(location_param = mean(location_param, na.rm = TRUE),
                  #Calculate midpoint, but I'm actually not sure if f(location_param) = midpoint
                  midpoint =  mean( calc_midpoint(chance_rate,lapse) ), 
                  .groups = "drop")
loc_param_each_group$curve_type <- "generating model"

#Draw eta line
hh<- hh + geom_segment(data = loc_param_each_group,
                   aes(x = location_param, xend = location_param,
                       y = 0, yend = midpoint), linetype="dotted"
                       ,alpha=.8)#, color = "grey3")

#Now add a line for the estimated location parameter
eta_Intercept <- fixef(fit)["eta_Intercept", "Estimate"]

# Calculate the midpoint for the current condition
midpoint_val <- mean(calc_midpoint(data_subset$chance_rate, lapse))

# Add the vertical segment at eta_Intercept to hh
hh <- hh +
  annotate( # useful to add segment with fixed values, not mapped from a data frame
    "segment",
    x = eta_Intercept, xend = eta_Intercept,
    y = 0, yend = midpoint_val,
    color = "red"
  )

#This removes the default padding/margin below the y-axis, so the plot area starts exactly at y = 0.
hh<-hh + scale_y_continuous(limits=c(0,1),expand = c(0, 0))
show(hh)
