...
subjPerGroup<- 25 #so 25 males and 25 females per lab*age_group
...
#| echo: false
 ...
 #MARKED FOR DELETION
  ...
  trials <- trials |> group_by(lab,age_group,gender,subjWithinGroup) |> 
...
lapse <- 0.05
sigma <- 0.2
sigma_between_participant_variance <- .001#0.2 
...
if ( as.numeric(subjectsTrials[1]) != trialsPerSubject ) {
...
data_simulated<- trials |> mutate( chance_rate = 1/obj_per_ring, location_param = 
          location_param_calculate( num_targets,age_group,gender,lab,subj_standardzd ) )

#Check num subjects not screwed up
...
gg<- gg + geom_hline( aes(yintercept = chance_rate), colour = "purple", alpha=0.2 ) +
     geom_hline( aes(yintercept = 1-lapse*(1-chance_rate)), colour = "yellow3", alpha=0.4 )
...
Hm, my formula doesn't seem to know about subjects. Does that mean it'll inapparopriately lump them together, resulting in the wrong estimate for the location parameter?

```{r}

my_nonlinear_predictor <-  correct ~ chance_rate + 
                                (1-chance_rate - lapseRate * (1-chance_rate)) * 
                                Phi(-(speed-eta)/exp(logSigma))

#Notes on Multi-level modeling. Two potential levels: lab and subject.
# Going all the way would mean having different effects of gender and age_group on eta in each lab.
# More common is to just have random intercepts for each lab and subject    (1 | lab/subjWithinGroup)
# eta ~ age_group + gender + lab +  (1 | lab/subjWithinGroup)

my_brms_formula <- brms::bf(
  my_nonlinear_predictor,
  eta ~ lab + age_group + gender + num_targets,  
...
See [visualize_and_select_priors.html](visualize_and_select_priors.html) for motivation and visualisation.
...
#set mc.cores option which is used by brms, parallel processes speeds up model 
...
## Understanding the brms estimand names

eta_Intercept is the eta for the older group

eta_age_groupolder is the eta offset for the older age group. 
eta_genderF is the eta offset for female.
 
The logSigma estimate tends to have wide confidence intervals.
logSigma_age_groupolder is the logSigma offset for the older age group.
...
trueValsNamesInBrmParlance<-
  c("eta_Intercept","eta_age_groupolder","eta_genderF", "eta_labRoudaia", "lapseRate_Intercept","logSigma_Intercept","logSigma_age_groupolder")
trueVal<-c( 
     trueVal_eta_young - age_penalty, age_penalty, gender_penalty, 
     Holcombe_lab_penalty, lapse, log(sigma), 0 )
trueVal<- data.frame(trueVal)
rownames(trueVal) <- trueValsNamesInBrmParlance

#Put estimates and trueVals in same dataframe
...
#pretty print estimates and discrepancies
estimates %>% dplyr::mutate(across(where(is.numeric), ~ round(.x, 2)))
```

The fit converged with no errors!!  (this doesn't always happen, when for example we use more complicated models)

With injection of subject heterogeneity, fit is not great and takes much longer
