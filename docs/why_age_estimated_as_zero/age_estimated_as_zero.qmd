...
#Ask brms for syntax for priors for my model
...
my_priors <- c(
   ...
   brms::set_prior(eta_prior_distribution, class = "Intercept", nlpar = "eta", lb = 0, ub = 2.5),
   brms::set_prior(logsigma_prior_distribution, class = "b", nlpar = "logSigma", lb = -2, ub = 1.6)
 )

#in an attempt to get eta_Intercept instead of Intercept_eta, try
my_priors <- c(
   brms::set_prior(lapseRate_prior_distribution, class = "b", nlpar = "lapseRate", lb = 0, ub = 1),
...
# define my priors, using alternative of prior_string, don't know how it's different.
...
#yeah because it's supposed to be eta_Intercept, not Intercept_eta?
```
...
chainchainchain_all <- ggs(fit, burnin = FALSE)
