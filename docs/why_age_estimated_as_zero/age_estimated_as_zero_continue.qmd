---
title: "What will it take to get age estimated correctly?"
format: html
---
...
laboratories<- factor( c("Roudaia") ) #, "Holcombe"),
                      # levels=c("Roudaia", "Holcombe") ) #defines the factor order
numTargetsConds<- factor( c("two", "three"),
                         levels=c("two", "three") ) #defines the factor order
...
target_penalty<-0.4 #penalty for additional target
age_penalty <- 0.4#0.4 #Old people have worse limit by this much
gender_penalty <- 0.12 #Female worse by this much
Holcombe_lab_penalty <- 0.4
...
location_param_calculate<- function(numTargets,ageGroup,gender,lab,
  ...
  after_penalties <-location_param - 
                      ...
                      if_else(ageGroup=="older",1,0) * age_penalty -
...
my_formula <- brms::bf( 
   correct ~ chance_rate + (1-chance_rate - .04*(1-chance_rate)) * Phi(-(speed-eta)/exp(log(0.2)))
  ) 
my_formula <- my_formula$formula

my_brms_formula <- brms::bf(
  correct ~ chance_rate + (1-chance_rate - .04 * (1-chance_rate))*Phi(-(speed-eta)/exp(log(0.2))), 
  eta ~ ageGroup + numTargets, #+ lab + numTargets,# + gender,
...
# define my priors
#"Intercept" means only applies to the intercept, so that coefficients can go negative if need to. But maybe I should make sure my reference group is "older" so then I can constrain that to be greater than zero
my_priors <- c( #brms::prior_string(lapseRate_prior_distribution, nlpar="lapseRate", 
          ...
          brms::prior_string(eta_prior_distribution, nlpar="eta", 
                            class = "b", #only applies to intercept
                            lb=0,ub=2.5) #,
...
# Combine all priors
priors_all <- bind_rows(#prior_lapse, prior_location, 
                        prior_scale)
...
time <- try( system.time({ 
  fit <-
    brm(
      ...
      #init = 0, #
      chains = 2,#3, #4 is the default to check consistent convergence
      iter = 400, #2000 is default number of iterations 
      save_warmup = TRUE
    ) 
  }), #end of system.time
...
eta_ageGroupyounger represents the eta advantage for the younger age group. 
...
trueValsNamesInBrmParlance<-
  c("eta_labRoudaia","eta_ageGroupolder","eta_numTargetstwo","eta_genderF", "lapseRate_Intercept","logSigma_Intercept") #,"logSigma_ageGroupyounger")
trueVal<-c( 
...
why is ageGroup estimates zero

Look at [what is happening during the chain](https://discourse.mc-stan.org/t/init-not-using-my-initial-values-and-seems-to-be-defaulting-to-0/39548)
...
```
Plot possibly- problematic ones (gender and age) specifically.
```{r}
#| echo: false 
chainchainchain |>
  filter(str_detect(Parameter, "age|gender")) |>
  filter(Iteration < 200) |> #Because nothing happens after first couple hundred
  ggplot(aes(x = Iteration, y = value, linetype = factor(Chain),
             color = Parameter)) +
  facet_grid(paramGroup ~ ., scales="free") +
  geom_line() +
  geom_hline(yintercept=age_penalty, alpha=.5,linetype="dotted")+
  geom_hline(yintercept=gender_penalty, alpha=.5,linetype="dotted")+
  ggtitle("")+
  theme_bw()+  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) #remove gridlines
...
chainchainchain |>
  filter( str_detect(Parameter, "age|lab") ) |>
  #filter(value < .4) |>
...
Look at warm-up iterations specifically.

```{r}
#| echo: false 

# Extract all samples, including warm-up
chain_of_fools <- ggs(fit, burnin = FALSE)

#guess warmup as half of total iterations if not available
warmup_iters <- 100# fit$fit@sim$warmup

# Plot all parameters, only for warm-up iterations
chain_of_fools |>
  filter(Iteration <= warmup_iters) |>
  filter( str_detect(Parameter, "age|lab") ) |>
  ggplot(aes(x = Iteration, y = value, linetype = factor(Chain), color = Parameter)) +
  facet_grid(Parameter~.) + 
  geom_line() +
  labs(
    title = "Warm-up Traceplots",
    x = "Iteration (Warm-up)",
    y = "Parameter Value"
  ) +
  theme_bw() + theme(panel.grid.minor = element_blank()) #remove gridlines
```