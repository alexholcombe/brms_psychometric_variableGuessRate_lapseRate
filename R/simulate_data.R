...
generate_conditions_both_labs<- function(labs,numSubjects,trialsPerCondition,targetNumConds,speeds) {
  #e.g.:
  #numSubjects<- 50
...
if (self_test) {
  ...
  trials <- generate_conditions_both_labs(laboratories,numSubjects,trialsPerCondition,targetNumConds,speeds)
  ...
  trialsDefinitelyUnbalanced<- trials |> filter( ! ( subj != "1" & num_targets != "2" & trialThisCond==1) ) 
  ...
  unbalanced<- trials %>% 
    group_by(lab, gender, age_group, subj, num_targets, obj_per_ring) %>%
    summarise(numGroups = n(), .groups = "drop") %>%
    count(numGroups)
  unbalanced <- nrow(unbalanced) > 1
  if (unbalanced) {
    print('unbalanced lab, gender, age_group, subj, num_targets, obj_per_ring = ',
          as.logical(unbalanced))
  }
  
  #Confirm that each condition has trials numbered 1 to trialsPerCondition
  groupedByCond<- trials |> group_by(lab,gender,age_group,subj,num_targets,obj_per_ring,speed) 
  ...
  trials <- trials |> group_by(lab,trialThisCond,age_group,gender,subj) |> 
