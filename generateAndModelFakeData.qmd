...
```{r}
predict_interval_brms <- predict(fit2, re_formula = NULL) #or use fitted to take into account uncertainty of observations
...
predict_interval_brms <- predict(fit2, newdata=conditionsUniq, re_formula = NULL) 
predictions<- cbind(conditionsUniq, predict_interval_brms)
